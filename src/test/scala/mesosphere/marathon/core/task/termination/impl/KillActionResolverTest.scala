...
class KillActionResolverTest extends UnitTest with TableDrivenPropertyChecks {
  ...
  def createToKill(instance: Instance) = ToKill(instance.instanceId, instance.tasksMap.keysIterator.to[Seq], Some(instance), attempts = 0)

  "computeKillAction" when {
    // format: OFF
    Table(
      ("name"                              , "instance",                  "expected"),
      ("an unreachable reserved instance"  , residentUnreachableInstance, KillAction.TransitionToReserved),
        ...
        case (name, instance, expected) =>
          s"killing ${name}" should {
            s"result in ${expected}" in {
              val toKill = createToKill(instance)
              KillActionResolver.computeAction(toKill).
                shouldBe(expected)
            }
          }
      }
