...
class RichCuratorFramework(val client: CuratorFramework) extends AnyVal {
  ...
  /**
    ...
    *
    * @param shutdownState reference to interface to query Marathon's lifecycle state
    */
  def blockUntilConnected(shutdownState: ShutdownState): Unit = {
    ...
    @tailrec def poll(): Unit = {
      ...
      else if (shutdownState.isShuttingDown)
